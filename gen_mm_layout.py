...
emit('    <mxGraphModel dx="1800" dy="1400" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="1860" pageHeight="1650">')
emit('      <root>')
emit('        <mxCell id="0"/>')
...
# ============================================================
container("reqBox", 20, 42, 1820, 100, "#F3E5F5", "#7B1FA2", sw="1.5")
...
# CN
text("cnReq", 60, 130, 1800, 16, "中文: 多模态请求携带mm_features(图片/视频/音频)，每个feature有唯一hash标识、在序列中的位置、embedding数量；调度时需检查编码器计算预算和缓存空间", fs=8, fc="#6A1B9A", align="left")

# ============================================================
# MAIN CONTAINERS: P-side and D-side
# ============================================================
container("Pbx", 20, 155, 910, 700, "#E3F2FD", "#1565C0")
container("Dbx", 950, 155, 890, 700, "#FFF3E0", "#E65100")

text("Ptt", 200, 163, 320, 24, "P Node (ec_producer + kv_producer)", fs=15, fst=1, fc="#1565C0")
text("Dtt", 1120, 163, 340, 24, "D Node (ec_consumer + kv_consumer)", fs=15, fst=1, fc="#E65100")

# ============================================================
# P-SIDE: Encoder Execution + EC Transfer
# ============================================================
container("pEncBox", 35, 200, 425, 310, "#E8EAF6", "#3949AB", dashed=True)
text("pEncT", 50, 205, 350, 18, "P: Encoder Execution (vision/audio model)", fs=13, fst=1, fc="#3949AB")

# P encoder flow
box("p1", 55, 235, 180, 38, "schedule():\nCheck encoder_inputs", fill="#C5CAE9", fc="#3949AB", fs=10)
box("p2", 55, 300, 180, 38, "_try_schedule_encoder_inputs()\nFind MM items in token window", fill="#C5CAE9", fc="#3949AB", fs=10)
diamond("p3", 90, 365, 150, 52, "Budget OK?\nCache space?", fill="#FFF9C4", fc="#F9A825", fs=10)
box("p4", 55, 448, 180, 44, "Run Encoder Forward\n(produce embeddings)\n→ save to local cache", fill="#A5D6A7", fc="#388E3C", fs=10)

text("cnPenc", 50, 498, 380, 30, "P中文: P节点负责运行视觉/音频编码器→产生embedding→存入EncoderCache→通过EC Connector传输给D", fs=7.5, fc="#283593", align="left")

# EC Connector on P side
container("ecPBox", 485, 200, 420, 310, "#E0F2F1", "#00695C", dashed=True)
text("ecPT", 500, 205, 350, 18, "EC Connector (P=Producer): Encoder Output Transfer", fs=13, fst=1, fc="#00695C")

box("ecP1", 505, 235, 185, 38, "Encoder complete:\nregister encoder outputs", fill="#B2DFDB", fc="#00695C", fs=10)
box("ecP2", 505, 300, 185, 38, "request_finished():\nmark for async send to D", fill="#B2DFDB", fc="#00695C", fs=10)
box("ecP3", 505, 365, 185, 38, "Worker: save_ec_layer()\n→ transfer via shared mem", fill="#80CBC4", fc="#004D40", fs=10)
box("ecP4", 505, 448, 185, 44, "get_finished():\nreport finished_sending\n→ free encoder cache", fill="#80CBC4", fc="#004D40", fs=10)

text("cnEcP", 500, 498, 380, 30, "EC Connector中文: P侧编码器完成后→EC Connector异步传输encoder outputs到D→类似KV Transfer但传输的是encoder embeddings", fs=7.5, fc="#004D40", align="left")
...
text("p3yL", 150, 425, 14, 14, "Y", fs=9, fc="#2E7D32")
# P encoder → EC connector
edge("p2ec", "p4", "ecP1", "#00695C", ex=1, ey=0.5, enx=0, eny=0.5, pts=[[280,470],[490,470],[490,254]])
...
# ============================================================
container("smbx", 340, 1225, 1180, 130, "#FAFAFA", "#BDBDBD")
text("smt", 780, 1230, 300, 20, "Multimodal Request Encoder State Transitions", fs=14, fst=1)

box("sm1", 360, 1268, 160, 38, "MM Item Pending\n(not yet encoded)", fill="#FFF9C4", fc="#F9A825", fs=10)
box("sm2", 600, 1268, 180, 38, "MM Item Scheduled\n(encoder forward queued)", fill="#BBDEFB", fc="#1976D2", fs=10)
box("sm3", 860, 1268, 180, 38, "MM Item Cached\n(embedding in cache)", fill="#C8E6C9", fc="#388E3C", fs=10)
box("sm4", 1120, 1268, 180, 38, "MM Item Loaded Async\n(from remote EC cache)", fill="#FFCC80", fc="#E65100", fs=10)
...
text("sm2L", 780,1273, 14,14, "完成", fs=9)
text("sm1rL", 485,1320, 50,14, "远端缓存", fs=9)
text("sm4rL", 1030,1315, 50,14, "加载完成", fs=9)

...
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-archive-keyring.gpg] https://mirrors.aliyun.com/kubernetes/apt/ kubernetes-xenial main" | tee /etc/apt/apt.conf.list.d/kubernetes.list
...
> **注意**：etcd 备份需要在每个控制节点上独立执行——每个节点的 etcd 数据是不同的（虽然是 Raft 一致性的相同数据，但快照文件在同一时间点等效），实际生产只选一台执行即可。

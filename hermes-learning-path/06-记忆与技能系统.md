...
graph TD
    subgraph MEMORY["记忆系统架构"]
        subgraph L1["第1层: 内置记忆 (文件系统)"]
            A["~/.hermes/MEMORY.md<br/>Agent 自己维护的持久事实"]
            B["~/.hermes/USER.md<br/>用户画像 (也由Agent维护)"]
            C["特点: 纯文本, 在System Prompt中直接注入, 会话间共享"]
        end
        subgraph L2["第2层: 外部记忆提供者 (插件)"]
            D["可选的第三方记忆系统, 通过 MemoryProvider 接口接入"]
            E["Honcho: 辩证式用户建模 (信念/偏好/目标)"]
            F["自定义提供者: 实现 MemoryProvider 抽象类"]
            G["严格限制: 最多同时激活1个外部提供者"]
        end
        subgraph L3["第3层: 会话搜索 (SQLite FTS5)"]
            H["~/.hermes/state.db 中的 messages_fts 虚拟表"]
            I["session_search() → 跨所有历史会话的全文搜索 + LLM 摘要"]
        end
        L1 --> L2 --> L3
    end
```
...
│   ├── DESCRIPTION.md          ← 分类描述
│   ├── pr-workflow/
│   │   └── SKILL.md            ← 核心: 技能指令 (YAML frontmatter + Markdown)
│   ├── code-review/
│   │   ├── SKILL.md
│   │   └── references/
│   │       └── checklist.md     ← 可选: 补充参考文档
│   └── auth/
│       └── SKILL.md
├── python/
│   ├── DESCRIPTION.md
...
```mermaid
graph TD
    subgraph CURATOR["Curator 维护规则"]
        A["1. 合并<br/>一次性技能 → 合并到更大的 umbrella 技能<br/>例如: fix-import-order + fix-type-hints → python-lint"]
        B["2. 归档 (不会删除)<br/>长时间未使用的技能 → 移到 .archive/ 目录"]
        C["3. 钉住 (Pinned)<br/>标记为 pinned 的技能永远不自动归档"]
        D["4. 只操作 Agent 自己创建的技能<br/>用户手动创建的技能不会被 Curator 改动"]
        E["5. 可恢复<br/>被归档的技能可以从 .archive/ 随时恢复"]
    end
```
...
```mermaid
graph LR
    subgraph RELATION["记忆与技能的关系"]
        M["记忆 (Memory)<br/>────────────<br/>存储'是什么'<br/>例如: 项目用FastAPI<br/>跨任务共享<br/>System Prompt注入<br/>轻量 (几KB)"]
        S["技能 (Skills)<br/>────────────<br/>存储'怎么做'<br/>例如: FastAPI项目的初始化步骤<br/>特定任务复用<br/>按需加载<br/>重量 (几KB-几十KB)"]
        M -->|"互补关系: 记忆提供上下文<br/>→ 技能利用上下文执行操作<br/>→ 产生新记忆"| S
    end
```

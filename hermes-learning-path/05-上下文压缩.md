...
```mermaid
flowchart LR
  n0["轮次1: terminal('cat main.py')              → 300行输出"]
  n1["轮次2: terminal('find . -name '*.py'')     → 50行输出"]
  n2["轮次3: web_search('Python async patterns')  → 10条搜索结果摘要"]
  n3["轮次4: read_file('config.py')               → 200行"]
  n4["轮次5: terminal('grep -r 'TODO' .')         → 80行"]
  n5["轮次6: web_extract('https://docs.python.org/...') → 500行"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
flowchart LR
  n0["Before (800 chars):"]
  n1["{'output': 'file1.py\nfile2.py\nfile3.py\n... (200 files) ...\n',"]
  n2["'exit_code': 0, 'duration_ms': 1500}"]
  n3["After (80 chars):"]
  n4["[terminal] ran ls -la → exit 0, 47 lines output"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4

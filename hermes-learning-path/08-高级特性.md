...
```mermaid
flowchart LR
  n0["用户: '帮我同时调研 Python 的三种 Web 框架: FastAPI, Litestar, Sanic'"]
  n1["主Agent的分析:"]
  n2["'这需要三个独立的调研任务，可以并行'"]
  n3["→ 创建3个子Agent，各自调研一个框架"]
  n4["→ 等待所有子Agent完成"]
  n5["→ 汇总结果"]
  n6["主Agent调用 delegate_task (3次, 并行):"]
  n7["子Agent 1: '调研FastAPI的最新特性、性能、社区活跃度'"]
  n8["子Agent 2: '调研Litestar的最新特性、性能、社区活跃度'"]
  n9["子Agent 3: '调研Sanic的最新特性、性能、社区活跃度'"]
  n10["每个子Agent独立运行完整的对话循环 (搜索web → 提取 → 分析)"]
  n11["→ 返回结构化结果给主Agent"]
  n12["→ 主Agent汇总: '根据调研，FastAPI在...方面最优...'"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
Agent 不会遇到错误就崩溃。`classify_api_error()` 将 API 错误分为 18 种类型:
...
class FailoverReason(Enum):
    ...
    llama_cpp_grammar_pattern = "llama_cpp_grammar_pattern"  # llama.cpp语法拒绝
...
```json
// ~/.hermes/auth.json 的 "credential_pool" 字段
[
  {
    "provider": "openrouter",
    "auth_type": "api_key",
...
tool_loop_guardrails:
  ...
  warn_after:
    exact_failure: 2           # 相同参数失败2次 → 警告
    same_tool_failure: 3       # 同类工具失败3次 → 警告
    idempotent_no_progress: 2  # 只读工具无进展2次 → 警告
  hard_stop_after:
    exact_failure: 5           # 相同参数失败5次 → 强制停止
    same_tool_failure: 8       # 同类工具失败8次 → 强制停止
    idempotent_no_progress: 5  # 只读工具无进展5次 → 强制停止
```
...
graph TD
    subgraph PATTERNS["核心设计模式回顾"]
        A["1. 策略模式 (Strategy Pattern)<br/>- 传输层: 4种API协议, 统一接口<br/>- 终端后端: 7种执行环境, 统一函数签名<br/>- 凭证选择: 4种分配策略<br/>- 记忆系统: 内置 + 外部提供者"]
...
下一章 [09-SDK与API开发接口](09-SDK与API开发接口.md) 介绍 Hermes 的 SDK 与编程接口。想要深入源码时，建议按以下顺序对照阅读:

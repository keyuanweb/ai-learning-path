...
### 3. CodexResponsesTransport

适配 OpenAI Responses API（Codex系列）。这个 API 的格式和前两个完全不同。

```python
class CodexResponsesTransport(ProviderTransport):
...
AWS Bedrock 的流式 API 完全不同，所以 AnthropicTransport 走 Anthropic SDK 的 `client.messages.stream()`，而 BedrockConverseTransport 走 `stream_converse_with_callbacks()`——但对外暴露的回调接口完全一致。

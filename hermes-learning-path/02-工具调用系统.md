...
但有些工具**必须串行执行**（比如前一个工具的结果是后一个工具的输入时）。Agent 用 `_should_parallelize_tool_batch()` 判断一批工具调用能否并发执行：命中黑名单工具（如 terminal）、参数解析失败、路径作用域重叠时都会退回串行。
...
`terminal_tool` 是 Hermes Agent 中最重要也最复杂的工具。它支持 **7 种后端**:

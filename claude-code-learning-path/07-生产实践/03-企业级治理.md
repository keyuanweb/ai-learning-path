...
```mermaid
flowchart TD
  n0["/etc/claude-code/managed-settings.json   ← 企业级（不可覆盖）"]
  n1["↓ 约束"]
  n2["~/.claude/settings.json                  ← 用户级（受企业策略限制）"]
  n3["↓ 约束"]
  n4[".claude/settings.json                    ← 项目级（受用户和企业限制）"]
  n5["↓ 约束"]
  n6[".claude/settings.local.json              ← 本地覆盖（不提交 Git）"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
```mermaid
flowchart TD
  n0["企业 MCP Gateway（集中认证 + 审计）"]
  n1["GitHub Enterprise Server"]
  n2["PostgreSQL (生产只读副本)"]
  n3["Internal API (经过审批的)"]
  n4["Playwright (受限沙箱)"]
  n0 --> n1
  n1 --> n2
  n2 --> n3

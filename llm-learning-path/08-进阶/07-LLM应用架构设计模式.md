...
```mermaid
flowchart LR
  n0["用户输入 → 分类器 → ├→ 简单问题 → 小模型 (7B, 便宜快)"]
  n1["一般问题 → 中等模型 (70B)"]
  n2["复杂推理 → 大模型 (GPT-4o/DeepSeek-V3)"]
  n0 --> n1
  n1 --> n2
```
...
flowchart LR
  n0["内容生成模型 (快速) → 输出草稿"]
  n1["评估模型 (挑剔)     → 对草稿打分/提意见"]
  n2["内容生成模型        → 根据反馈修改 → 输出终稿"]
  n0 --> n1
  n1 --> n2
```
...
```mermaid
flowchart TD
  n0["通用模型接收请求 → 发现需要代码 → 转发给代码专项模型 → 汇总结果"]
```
...
```mermaid
flowchart TD
  n0["同一个问题发给 3 个不同模型 → 得到 3 个回答 → 用第 4 个模型来综合/投票"]
```
...
flowchart TD
  ...
  n2["[输入安全 Guard]"]
  n3["↓"]
  n4["[路由 Router]  →  选择合适的模型"]
  n5["↓"]
  n6["[缓存 Cache]   →  精确匹配 → 直接返回"]
  n7["↓"]
  n8["[LLM 调用]     →  主/备/本地 降级链"]
  n9["↓"]
  n10["[输出安全 Guard]"]
  n11["↓"]
  n12["[流式返回]"]
  n13["↓"]
  n14["[日志/监控]    →  记录延迟、成本、错误"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4

...
logits = torch.tensor([-1.0, 0.5, 3.0, 1.2])  # 4 个词的原始分数
...
**记住这个就够了**：大模型的输入接口是 Tokenizer（文字→数字），输出接口是 Softmax+采样（数字→文字）。语言模型的本质是"猜下一个词"，上下文学习到的一切高级能力（推理、翻译、编程）都是这个简单任务的副产品。

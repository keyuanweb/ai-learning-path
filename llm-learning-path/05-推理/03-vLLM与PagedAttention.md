...
```mermaid
flowchart LR
  n0["传统: 每个请求 = 一大块连续内存 (max_len)"]
  n1["PagedAttention: 每个请求 = 多个不连续的小页 (page_size, 比如 16 tokens)"]
  n2["请求 A: [页0] [页1] [页2] → 只需要 48 个 token 的 KV"]
  n3["请求 B: [页4] [页7] → 只需要 32 个 token 的 KV"]
  n4["请求 C: [页3] [页5] [页6] [页8] → 需要 64 个 token 的 KV"]
  n5["空闲: [页9] [页10] ... 可以被新请求使用"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
```mermaid
flowchart LR
  n0["批次1: [请求A(短), 请求B(长), 请求C(短)]"]
  n1["→ 必须等 B 也完成才能返回结果给 A 和 C"]
  n2["→ A 和 C 的算力在白白等待"]
  n0 --> n1
  n1 --> n2
```
...
```mermaid
flowchart LR
  n0["时刻1: Batch [A, B, C, D]  开始"]
  n1["时刻2: A 完成了 → Batch [B, C, D]  (A 的结果立即返回)"]
  n2["时刻3: E 到达 → Batch [B, C, D, E] (新请求无缝加入)"]
  n3["时刻4: C 完成了 → Batch [B, D, E]"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
```

...
```
   ...
   ┌─────────────────────┐    ┌─────────────────────┐    ┌─────────────────────┐
   │  独立的 vLLM 进程   │    │  独立的 vLLM 进程   │    │  独立的 vLLM 进程   │
   │  使用 AR Worker      │    │  使用 Generation     │    │  使用 Generation     │
   │  KV Cache 管理       │    │  Worker              │    │  Worker              │
   │  输出: token+embed   │───▶│  输出: 声学特征      │───▶│  输出: 音频波形      │
   └─────────────────────┘    └─────────────────────┘    └─────────────────────┘
```
...
```
请求 A: [Thinker────] [Talker────] [Code2Wav──]
请求 B:              [Thinker────] [Talker────] [Code2Wav──]
请求 C:                            [Thinker────] [Talker────] [Code2Wav──]
                    ↑ 时间重叠，吞吐量提升 ↑
```

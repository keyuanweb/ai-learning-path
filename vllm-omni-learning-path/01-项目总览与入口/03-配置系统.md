# 03 · 配置系统：OmniModelConfig 与 pipeline_registry
...
| `stage_connector_config` | `{"name":"SharedMemoryConnector","extra":{}}` | Stage 间连接器配置 |
...
## pipeline_registry —— 模型到 Stage 的映射

[`pipeline_registry`](../../code/vllm-omni/vllm_omni/config/pipeline_registry.py) 定义了"哪个模型有几个 Stage、每个 Stage 是什么类型"。它通过每个模型目录下的 `pipeline.py`（定义 `PipelineConfig`）在 `pipeline_registry.py` 中集中登记；`model_executor/stage_configs/` 下另存有部分 YAML 配置。
...
当引擎启动时，它会根据 `pipeline_registry` 中的 Pipeline 配置知道需要创建几个 `StagePool`、每个 Stage 用什么 Worker。

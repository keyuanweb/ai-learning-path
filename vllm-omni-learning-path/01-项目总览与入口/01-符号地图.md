...
2. **打补丁（patch）**：vLLM-Omni 需要修改 vLLM 的一些行为。比如把 vLLM 的 `TokensPrompt` 替换成 `OmniTokensPrompt`（`Request`、`EngineCoreOutput` 等同理）
...
vLLM-Omni 依赖 vLLM 但不捆绑。它的 `setup.py` 会根据你的硬件平台（CUDA/ROCm/NPU/XPU）自动安装对应的平台依赖。

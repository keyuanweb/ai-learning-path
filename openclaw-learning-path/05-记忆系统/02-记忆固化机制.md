...
**摘要示例**：
```
输入: 30 轮对话（用户请求搭建 Next.js 项目 → 遇到 Tailwind 配置问题 → 解决）
输出: 3 段 Markdown 摘要（约 500 字）
压缩比: ~95%
```
...
```
1. 精确匹配 MEMORY.md 中的相关条目（关键词/标签匹配）
2. 语义搜索 Vector DB 中的相关日志片段
3. Graph DB 中关联的记忆链（如 "项目A → 服务器 → 部署流程"）
4. 优先注入最近的 + 最常引用的记忆
```

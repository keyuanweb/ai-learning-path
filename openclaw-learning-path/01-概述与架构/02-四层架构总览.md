...
```mermaid
flowchart LR
  n0["第 1 层: Session Context   → 当前会话的完整上下文（最近 N 轮对话）"]
  n1["第 2 层: Daily Logs        → 每日会话摘要日志（自动生成）"]
  n2["第 3 层: MEMORY.md         → Agent 主动筛选的长期记忆（结构化 Markdown）"]
  n3["第 4 层: Vector Search     → 语义向量检索（混合 Redis + Vector DB + Graph DB）"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
```
...
flowchart LR
  n0["用户消息（Telegram）"]
  n1["→ Channel Adapter 协议翻译"]
  n2["→ Gateway 鉴权 + 会话关联"]
  n3["→ Pi Agent Runtime 接收"]
  n4["→ Memory 注入相关记忆到上下文"]
  n5["→ System Prompt 组装"]
  n6["→ LLM Provider API 调用"]
  n7["→ 响应解析"]
  n8["→ 工具调用? ──Yes──→ Tool Execution ──→ 结果注入循环"]
  n9["(沙箱/Shell/Browser)"]
  n10["No──→ 响应格式化 → Channel Adapter → 用户收到回复"]
  n11["同时触发:"]
  n12["→ Daily Log 写入"]
  n13["→ 记忆筛选与固化"]
  n14["→ Heartbeat 检查（是否有定时任务待执行）"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4

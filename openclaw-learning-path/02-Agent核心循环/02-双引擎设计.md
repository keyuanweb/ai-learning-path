...
flowchart TD
  n0["规划引擎分解:"]
  n1["T1: git clone <repo-url>                    [工具: Shell]"]
  n2["T2: 统计代码行数和文件结构                    [工具: Shell]"]
  n3["T3: 运行 lint 工具检查代码规范                [工具: Shell]"]
  n4["T4: 分析依赖关系和过期包                      [工具: Shell]"]
  n5["T5: 搜索已知漏洞                            [工具: WebSearch + WebFetch]"]
  n6["↓ (依赖 T2, T3, T4, T5)"]
  n7["T6: 汇总分析结果，生成 Markdown 报告           [工具: FileWrite]"]
  n8["↓ (依赖 T6)"]
  n9["T7: 发送邮件到 user@example.com             [工具: Email/SendGrid]"]
  n10["执行顺序: T1 → [T2, T3, T4, T5]并行 → T6 → T7"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
详细内容见[阶段 7: 多 Agent 协作](../07-多Agent协作/)

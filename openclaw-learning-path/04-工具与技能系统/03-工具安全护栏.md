...
│   第 2 层: 参数扫描 (Parameter Scanning)               │
│   第 3 层: 人工审批 (Human-in-the-Loop)                │
│   第 4 层: 沙箱隔离 (Sandbox Isolation)                │
│                                                       │
│   每一层都可以独立拦截，也可以组合使用                    │
└──────────────────────────────────────────────────────┘
```
...
```
Agent 请求: bash("git push origin main")

1. 按顺序匹配白名单规则
2. 命中 "git (status|log|diff|add|commit|branch|checkout)" → 不匹配 push
3. 未命中任何允许规则 → 检查是否有禁止规则
4. 未命中禁止规则 → 默认拒绝（default-deny 策略）
5. 返回: "Command 'git push origin main' not in allowlist. Request approval?"
```
...
Agent 请求高风险操作
  → Gateway 暂停会话处理
    → 向管理员发送审批请求（含操作详情）
      → 管理员在 Telegram/Discord/Dashboard 上 Approve / Deny
        → Approve: 继续执行
        → Deny: 拒绝并告知 Agent
        → Timeout: 自动拒绝
```

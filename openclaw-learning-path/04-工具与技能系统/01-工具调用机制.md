...
│   Shell/CLI    │   bash / exec / spawn / kill             │
│   浏览器        │   browser_navigate / click / type /      │
│                │   screenshot / evaluate                   │
│   Web          │   web_search / web_fetch / api_call       │
│   Docker       │   docker_run / docker_list / docker_kill  │
│   Agent 管理   │   sessions_spawn / sessions_list          │
│   Workspace    │   workspace_read / workspace_edit         │
│   多媒体        │   media_play / tts_speak / image_generate │
│   通信          │   email_send / message_send              │
│   系统          │   cron_schedule / heartbeat_check        │
└────────────────┴─────────────────────────────────────────┘
```
...
```
read   → 读取文件内容（支持分页、行范围）
write  → 创建或覆写文件（需要 read 过的路径验证）
edit   → 精确字符串替换（需要 old_string 在文件中唯一）
glob   → 按模式匹配文件路径
grep   → 按正则/关键词搜索文件内容
```
...
```
web_search(query, n=10)    → 搜索并返回结果列表
web_fetch(url, prompt)     → 抓取网页并用 LLM 提取指定信息
```

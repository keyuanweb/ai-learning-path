...
│   威胁类别      │   具体风险                                    │
├────────────────┼─────────────────────────────────────────────┤
│   命令注入      │   用户消息中的恶意命令（通过 Agent 间接执行）     │
│   提示词注入    │   恶意网页内容触发 Agent 的危险行为              │
│   凭证窃取      │   Agent 访问 .env/.pem 等敏感文件              │
│   沙箱逃逸      │   Docker 配置不当导致的容器逃逸                 │
│   供应链攻击    │   恶意 ClawHub 技能/插件                        │
│   数据泄露      │   记忆中的敏感信息通过 LLM API 外传             │
│   未授权访问    │   Gateway 暴露到公网 + 弱鉴权                   │
│   会话劫持      │   WebSocket 连接被窃取（CVE-2026-25253）       │
└────────────────┴─────────────────────────────────────────────┘
```
...
用户/攻击者
    ...
    ▼
┌──────────────────────┐
│  第 0 层: 传输安全    │  TLS + 设备配对 + Session Token
├──────────────────────┤
│  第 1 层: 身份认证    │  RBAC (Admin/User/Guest) + Device Pairing
├──────────────────────┤
│  第 2 层: 护栏过滤    │  命令白名单 + 参数扫描 + 敏感路径拦截
├──────────────────────┤
│  第 3 层: 沙箱隔离    │  Docker Namespace + Cgroup + Seccomp + AppArmor
├──────────────────────┤
│  第 4 层: 审计追踪    │  全操作日志 + OpenTelemetry Trace
└──────────────────────┘
```
...
```
新设备: "请求接入 Session alice-default"
Gateway: 生成挑战码 → 发送到已配对设备（如 Telegram）
已配对设备: 用户看到挑战码 → 确认 → 新设备授权成功
```
...
宿主机
  ...
  ├── Pi Agent 进程
  └── Docker Daemon
        ├── Container: openclaw-session-alice
        │   ├── 只读挂载: workspace/files/ (--read-only)
        │   ├── 临时写入: /tmp (tmpfs)
        │   ├── 网络: --net=none（默认）
        │   ├── 权限: --cap-drop=ALL（去除所有 Linux Capability）
        │   ├── Seccomp: 白名单系统调用
        │   └── Cgroup: CPU 2核 / 内存 2GB / 磁盘 10GB
        │
        └── Container: openclaw-subagent-coding
              └── ...（更严格的限制）
```

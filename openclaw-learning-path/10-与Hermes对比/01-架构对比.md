...
```
用户消息 → Channel Adapter → Gateway (单写入者) → Pi Agent (ReAct)
                                                        ↓
                                            Memory (四层检索)
                                            Skill (懒加载)
                                            Tool (执行引擎)
...
```
用户消息 → CLI/TUI 或 Gateway → AIAgent 核心 (run_agent.py 主循环)
                                        ↓
                              System Prompt 组装 (prompt_builder.py)
                              记忆注入 (memory_manager.py)
                              工具调度 (model_tools.py)
                                        ↓
                              传输层适配 → LLM API → 响应
```

...
| `execute_model(scheduler_output)` | 跑一次模型前向。可能返回 `ModelRunnerOutput`（尾 rank）或 `IntermediateTensors`（中间 PP rank）或 None（采样延迟） |
...
| `determine_available_memory()` | 通过哑加载（dummy load）profile 可用 GPU 内存 |
| `shutdown()` | 清理 CUDA 资源 |
...
| Worker 类 | 文件 | 设备 |
|-----------|------|------|
| `GPUWorker` | `v1/worker/gpu/worker.py` | NVIDIA GPU |
| `CPUWorker` | `v1/worker/cpu/worker.py` | CPU |
| `TPUWorker` | `v1/worker/tpu/worker.py` | TPU |

GPUWorker 是默认也是最常用的。CPU/TPU Worker 第一遍忽略。

## 阅读重点

- 理解 `execute_model` 和 `sample_tokens` 的「合并 vs 分开」两种模式
- 知道 `WorkerWrapperBase` 的懒初始化模式：先创建 wrapper，后加载模型
- 第一遍只看 GPUWorker 的接口，不看 CPU/TPU

...
- **通用 Transformers 后端**：如果模型不在注册表中但有 HF config，使用 `TransformersForCausalLM`（transformers 后端）自动生成支持
- **变体规范化**：例如 `XxxForConditionalGeneration` → `XxxForCausalLM`（后缀自动替换）
- **JSON 缓存**：已解析的模型注册输出缓存在 `~/.cache/vllm/modelinfos/` 中，加速后续加载
...
`initialize_model`：`ModelRegistry.resolve_model_cls()` → 实例化如 `LlamaForCausalLM`。**load_weights** 迭代 checkpoint，`AutoWeightsLoader` 按 `packed_modules_mapping` 处理 HF 融合布局。
...
4. 对于融合参数（如 `q_proj`），根据 `packed_modules_mapping` 确定在融合 weight 中的切片位置，然后 `fused_weight[offset:offset+size].copy_(q_proj_weight)`
...
- 理解 `packed_modules_mapping` 在权重加载中的作用

...
```
GPU 0: [ColumnParallel: 持 qkv_proj 前 1/4 列]
       → 局部输出 (1/4)
       → [RowParallel: 持 o_proj 前 1/4 行]
       → all-reduce → 完整输出

GPU 1: [ColumnParallel: 持 qkv_proj 第 2/4 列]
       → 局部输出 (1/4)
       → [RowParallel: 持 o_proj 第 2/4 行]
       → all-reduce → 完整输出
```
...
PP 的关键优化在 `EngineCore.step_with_batch_queue()`：
```
时间: ────────────────────────────────────────────────────→
Batch A: [schedule] [GPU0:layers 0-15] [GPU1:layers 16-31] [sample]
Batch B:            [schedule]        [GPU0:layers 0-15]    [GPU1:layers 16-31]
Batch C:                              [schedule]            [GPU0:layers 0-15]
```

通过 pipeline 让 CPU 调度和 GPU 执行重叠，接近 100% 设备利用率。

...
```
1. compile_grammar(StructuredOutputsParams)
   → 编译 JSON schema / regex 为 XGrammar 内部 FSM
   
2. 每步在 EngineCore.step() 中：
   a. scheduler.get_grammar_bitmask(scheduler_output)
      → 对于每个有 grammar 的请求：
         根据请求当前已生成的 token 序列，更新 FSM 状态
         调用 fill_bitmask() 填充一个 bitmask tensor
         bitmask[i] = 1 → token i 在当前语法下合法
         bitmask[i] = 0 → token i 被禁止
   
   b. sampler.sample_tokens(logits, bitmask)

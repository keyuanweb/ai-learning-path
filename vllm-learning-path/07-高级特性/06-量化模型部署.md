...
| **消费级 (RTX 4060/4070)** | 8-12GB | AWQ + 8-bit KV cache | 13B-30B 模型 |
...
在线量化的配置定义在 [`code/vllm/vllm/config/quantization.py`](../../code/vllm/vllm/config/quantization.py) 的 `QuantizationConfigArgs` 中。
...
TurboQuant 部署时首尾各 2 层自动加入 skip 列表——引擎在 [`arg_utils.py:1926-1935`](../../code/vllm/vllm/engine/arg_utils.py#L1926) 中处理。
...
5. **已废弃方法**（`fbgemm_fp8`, `fp_quant`）需 `--allow-deprecated-quantization` 才能使用——不推荐。

...
**源码**：[`code/vllm/vllm/v1/kv_offload/tiering/cpu/`](../../code/vllm/vllm/v1/kv_offload/tiering/cpu/)
...
flowchart TD
  full["GPU 显存满"]
  select["EvictionPolicy.select_victims()"]
  lru["LRU: 淘汰最久未访问"]
...
def create_offload_manager(vllm_config: VllmConfig) -> KvOffloadManager:
    backend_type = vllm_config.cache_config.kv_offload_backend  # "cpu" | "p2p" | "fs"
    policy_type = vllm_config.cache_config.kv_offload_policy     # "lru" | "arc"
    
    backend = BACKEND_REGISTRY[backend_type](vllm_config)
    policy = POLICY_REGISTRY[policy_type]()
    return KvOffloadManager(backend, policy)
```
...
```python
class AsyncLookup:
    def submit(self, block_ids: list[int]) -> Future[set[int]]:
        """提交查询请求，返回存在的 block_id 集合"""
    
    def get_result(self, future: Future) -> set[int]:
        """获取查询结果（非阻塞）"""
```
...
- `tiering/cpu/manager.py`——CPU backend 的 swap 决策

...
| `DeepSeekV4ParserReasoningAdapter` | DeepSeek V4 | Engine-native reasoning stream |
| `Qwen3ParserReasoningAdapter` | Qwen3 | Engine-native reasoning events |
| `Gemma4ParserReasoningAdapter` | Gemma 4 | `...` |
| `InklingReasoningParser` | Inkling | `response` |
| `GptOssReasoningParser` | GPT-OSS | Engine-native streaming |
...
_REASONING_PARSERS_TO_REGISTER: dict[str, tuple[str, str]] = {
    ...
    "deepseek_v4": ("deepseek_v4_engine_reasoning_parser", "DeepSeekV4ParserReasoningAdapter"),
    ...
}
...
class ReasoningParserManager:
    ...
    def get_reasoning_parser(cls, name: str) -> type[ReasoningParser]:
        # name（如 "deepseek_r1"）由模型配置推断，按 _REASONING_PARSERS_TO_REGISTER 懒加载
```
...
**源码**：[`code/vllm/vllm/tool_parsers/abstract_tool_parser.py`](../../code/vllm/vllm/tool_parsers/abstract_tool_parser.py)（增量 JSON 工具见 [`streaming.py`](../../code/vllm/vllm/tool_parsers/streaming.py)）
...
如果模型使用 engine-based streaming（`engine_based_streaming = True`），引擎在生成过程中直接产生结构化事件，无需后解析。

## 与 Rust Parser 的关系

Rust workspace 中的 `vllm-parser` ([`rust/src/parser/`](../../code/vllm/rust/src/parser/)) 提供了高性能的 Rust 实现：

- `DeepSeekV32Parser`：DeepSeek V3.2 的 reasoning + tool call 解析（Python，`vllm/parser/deepseek_v32.py`）
- `Gemma4Parser`：Gemma 4 的结构化输出解析（Python，`vllm/parser/gemma4.py`）
- Rust 的 tool call 解析通过 PyO3 导出到 Python（`vllm._rust_tool_parser`，见 `rust/src/parser/python/`）

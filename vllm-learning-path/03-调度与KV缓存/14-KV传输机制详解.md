...
flowchart TB
    ...
    subgraph IMPL["具体实现"]
        NIXL["NixlBaseConnector<br/>RDMA GPU-Direct"]
        MOON["MooncakeConnector<br/>RDMA + Store"]
        FLEX["FlexKVConnectorV1<br/>弹性缓存"]
        LMC["LMCacheConnectorV1<br/>共享缓存"]
...
| Connector | 传输方式 | 适用场景 |
|-----------|---------|---------|
| **NixlPullConnector** | RDMA READ (D 主动拉) | 标准 PD 分离 |
...
sequenceDiagram
    ...
    W->>W: 对每个 reqs_to_recv 中的请求:<br/>- 检查是否已握手<br/>- 未握手: _background_nixl_handshake()<br/>- 已握手: _read_blocks_for_req()
    W->>W: _compute_desc_ids(local_blocks)<br/>将逻辑 block_id → NIXL descriptor ID
    W->>NIXL: make_prepped_xfer("READ",<br/>local_handle, local_desc_ids,<br/>remote_handle, remote_desc_ids)
    ...
    loop 检查每个异步 handle
        W->>NIXL: check_xfer_state(handle)
        NIXL-->>W: done / pending
...
```
_read_blocks_for_req() 执行流程:
─────────────────────────

① 计算 descriptor IDs
   _compute_desc_ids(block_ids, dst_num_blocks, block_size_ratio, physical_blocks_per_logical)
...
Cross-Layer 模式 (cross_layers_blocks=True):
  ...
  前提条件:
    ...
    ③ TransferTopology 检测到 cross_layers_blocks=True（KV tensor 多出 num_layers 维度）
```
...
update_from_output() ◄────────────────── get_finished()
  ...
  _try_promote_blocked_waiting_request()   → invalid_block_ids

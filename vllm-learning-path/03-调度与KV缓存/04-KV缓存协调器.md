...
```
KVCacheManager (facade, 对调度器暴露)
  └─ KVCacheCoordinator (跨 group 协调)
       ├─ SingleTypeKVCacheManager["full_attn"]       # FullAttention KV cache
       │     └─ BlockPool[full_attn]                   #   该类型的块池
       ├─ SingleTypeKVCacheManager["mla"]              # MLA KV cache
       │     └─ BlockPool[mla]
       └─ SingleTypeKVCacheManager["sliding_window"]   # SlidingWindow KV cache
             └─ BlockPool[sliding_window]
```
...
| 实现 | 文件 | 说明 |
|------|------|------|
| `NoPrefixCacheCoordinator` | `no_prefix_cache_coordinator.py` | 无 prefix cache，简单 FIFO |
| `UnitaryKVCacheCoordinator` | `unitary_kv_cache_coordinator.py` | 统一块池，支持 prefix cache，适合单一 attention 类型的模型 |
| `HybridKVCacheCoordinator` | `hybrid_kv_cache_coordinator.py` | 混合 coordinator，支持多种 attention 类型 + prefix cache |
...
KV 块组织成**单向链表**结构：
```
Request A: [Block#3] → [Block#7] → [Block#12] → [Block#5]
                           ↖
Request B: [Block#3] → [Block#7] → [Block#20]
                           (共享！ref_count=2)
```

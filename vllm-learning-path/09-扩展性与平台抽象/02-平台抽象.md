...
class Platform:
    # 平台标识
    name: str                         # 如 "cuda", "rocm", "tpu"
    device_type: str                  # torch device type
    device_name: str                  # 显示名称
    platform_enum: PlatformEnum       # 枚举值
    
    # 硬件能力
    is_cuda_alike: bool               # CUDA-like 行为（大部分 NVIDIA/AMD GPU）
    inference_mode: InferenceMode     # 推理模式
    
    # 组件选择
    def get_attn_backend_cls(...) -> type[AttentionBackend]: ...
    def get_device_communicator_cls(...) -> type: ...
    def get_custom_op_cls() -> type: ...
    
    # Device 操作
    def is_device_supported(device) -> bool: ...
...
| **Custom Op** | `current_platform.get_custom_op_cls()` 返回平台特定 op 实现 |
| **Worker** | `get_worker_class()` 根据平台选择 GPUWorker/CPUWorker/TPUWorker/XPUWorker |

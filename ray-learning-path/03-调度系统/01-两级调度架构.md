...
flowchart TD
  ...
  n2["Node2: [■■■□□□□□□□]  33%"]
  n3["Node3: [■■■□□□□□□□]  33%"]
  n4["→ 新 Task 优先去负载最低的节点"]
  n5["DEFAULT:"]
  n6["优先考虑：数据本地性 > 资源可用性 > 负载均衡"]
  n7["新 Task 优先调度到参数（ObjectRef）所在的节点"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
```mermaid
flowchart LR
  n0["节点 Worker 池 (以 CPU worker 为例)"]
  n1["[idle]  [idle]  [busy]  [idle]  [busy]│"]
  n2["空闲 Worker 数量: 3                   │"]
  n3["总 Worker 数量: 5                     │"]
  n4["┤"]
  n5["新 Task 到达：                         │"]
  n6["if 有空闲 Worker → 立即分配            │"]
  n7["elif Worker < num_cpus → 启动新 Worker │"]
  n8["else → 进入等待队列                    │"]
  n0 --> n1
  n1 --> n2
  n2 --> n3

...
```
1. CREATE → PG 对象创建，请求发送到 GCS
2. RESCHEDULING → GCS 寻找满足条件的节点组合
3. CREATED → 资源预留成功，await pg.ready() 返回
4. REMOVED → ray.util.remove_placement_group(pg) 或超出作用域
```
...
```
SPREAD: 每个 bundle 在不同节点
  → GCS 按顺序尝试将 bundle[i] 放到节点 i
  → 节点 i 资源不够 → bundle[i] 跳过，尝试下个节点
  → 全部 bundle 都被分配到不同节点 → 成功
  → 有 bundle 分配失败 → 整个 PG 进入 RESCHEDULING
```

...
```
1. 看 Dashboard → 资源利用率
   GPU < 50%? → 增加 batch size 或并行度
   CPU < 20%? → 可能是 IO 瓶颈或序列化瓶颈

2. 看 Timeline → Task 时间分布
   等待时间长? → 调度或数据传输问题
   执行时间长? → 代码本身需要优化
   两者都长? → 可能是对象序列化瓶颈

3. 看 Object Store → 使用率和溢出
   溢出频繁? → 增加 object_store_memory
   使用率持续上升? → 可能有内存泄漏
...
```
每次优化后，重新跑一遍基准测试
→ 确认"优化"真的带来了提升（有时适得其反）
```

### 3. 只有单节点基准，没测多节点

```
序列化瓶颈在单节点不明显（零拷贝掩盖了问题）
→ 一定要在多节点环境下测试性能
```
